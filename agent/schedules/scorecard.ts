...
const JUDGE_PASS = [
  ...
  "2. For EACH entry in `cycles`, delegate to the `report_judge` subagent. Put the report text and the whole `groundTruth` object in `message`, VERBATIM and complete, including its `coverage` notes (they say which categories that cycle's ground truth can and cannot adjudicate, so dropping them makes correct reports look invented). State in the message that the ground truth is the ONLY source of truth and that it is grading, not rewriting. Set `outputSchema` to {\"type\":\"object\",\"properties\":{\"grounding\":{\"type\":\"integer\"},\"consistency\":{\"type\":\"integer\"},\"calibration\":{\"type\":\"integer\"},\"completeness\":{\"type\":\"integer\"},\"overall\":{\"type\":\"integer\"},\"findings\":{\"type\":\"array\",\"items\":{\"type\":\"string\"}}},\"required\":[\"grounding\",\"consistency\",\"calibration\",\"completeness\",\"overall\"]}.",
